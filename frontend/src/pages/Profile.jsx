...
const C = {
  ...
  borderGold:  'rgba(212,170,60,0.28)',
  gold:        '#d4aa3c',
  goldBright:  '#f0cc6a',
  goldDim:     '#7a6020',
  white:       '#efefef',
  whiteDim:    'rgba(239,239,239,0.55)',
  red:         '#a83230',
};
...
$s.textContent = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  /* entry animations only — no looping motion except gold shimmer */
  @keyframes fadeUp {
  ...
  /* slow gold shimmer on display text */
  @keyframes goldShimmer {
    0%   { background-position:-300% center }
    100% { background-position: 300% center }
  }

  /* one-shot haki pulse on avatar ring — settles to faint glow */
  @keyframes crackGlow {
  ...
  .anime-card:hover {
    ...
    box-shadow:0 26px 56px rgba(0,0,0,.88), 0 0 30px rgba(212,170,60,.13) !important;
    border-color:rgba(212,170,60,.40) !important;
  }
  ...
  input:focus, textarea:focus, select:focus {
    outline:none;
    border-color:rgba(212,170,60,.52) !important;
    box-shadow:0 0 0 2px rgba(212,170,60,.07) !important;
  }
...
const inputBase = {
  ...
  borderRadius:2, textTransform: 'none', padding:'11px 14px', color:C.white, fontFamily:"'Cinzel',serif",
  fontSize:'.82rem', transition:'border-color .2s, box-shadow .2s',
};
...
export default function Profile() {
  ...
  const onLogout = () => { localStorage.removeItem('userId'); navigate('/'); };
...
const onSave = async () => {
    try {
      ...
      // 🎯 ADDED: The third argument { withCredentials: true }
  ...
  const onUpdateStatus = async (anime) => {
    try {
      ...
      await axios.post('http://localhost:5000/api/watchlist/add', {
        userId:loggedInUserId, animeId:anime.animeId, title:anime.title, image:anime.image, genres:anime.genres, status,
      });
  ...
  return (
    <div style={{
      ...
      <div className="enter-profile" style={{
        ...
        border:`1px solid rgba(212,170,60,.18)`,
        boxShadow:'0 32px 80px rgba(0,0,0,.92), 0 0 0 1px rgba(255,255,255,.02)',
        overflow:'hidden',
      }}>
        ...
        {!isEditing && (
          <div style={{
            ...
            background:'linear-gradient(180deg,rgba(212,170,60,.04) 0%,transparent 100%)',
          }}>
          ...
          {!isEditing ? (
            <>
              ...
              <div style={{
                ...
                borderTop:`1px solid ${C.borderGold}`, borderBottom:`1px solid ${C.borderGold}`,
                background:'rgba(212,170,60,.03)',
              }}>
              ...
              <p style={{
                ...
                fontSize:'1.02rem', lineHeight:1.78, color:C.whiteDim,
                textAlign:'center', margin:'0 0 26px', padding:'0 6px',
              }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:15 }}>
              ...
              <div style={{ display:'flex', gap:10, justifyContent:'center', marginTop:4 }}>
                ...
                <button className="btn" style={mkBtn('ghost')} onClick={() => setIsEditing(false)}>Cancel</button>
              </div>
            </div>
          )}
        </div>
...
<div style={{ 
  ...
  top: 10, 
  left: 10, 
  padding: '5px 10px',        // Slightly thicker padding
  background: s.bg,           // Keeps your dynamic background color!
  backdropFilter: 'blur(8px)',// Adds the frosted glass effect
  WebkitBackdropFilter: 'blur(8px)', // Safari support for the blur
  border: `1px solid ${s.border}`, 
  color: s.color, 
  fontSize: '.8rem',          // Bumped up slightly for readability
  letterSpacing: '2px', 
  fontWeight: 800, 
  textShadow: '0 2px 5px rgba(0,0,0,0.9)', // Ensures text pops on light backgrounds
  borderRadius: '4px',        // Smooth modern corners
  boxShadow: '0 4px 15px rgba(0,0,0,0.6)', // Dropshadow separating the badge from the image
  zIndex: 10 
}}>
  {s.label}
</div>
                    ...
                    {isOwnProfile && (
                      <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
                        ...
                        <div style={{ display:'flex', gap:7 }}>
                          <button className="btn" onClick={() => onUpdateStatus(anime)}
                            style={{ ...mkBtn('mini'), flex:1, color:C.gold, borderColor:'rgba(212,170,60,.28)' }}>Update</button>
                          <button className="btn" onClick={() => onRemove(anime.animeId)}
                            style={{ ...mkBtn('mini'), color:'rgba(168,50,48,.85)', borderColor:'rgba(168,50,48,.28)' }}>Remove</button>
                        </div>
                      </div>
                    )}
                  </div>
