...
export default function Profile() {
  ...
  const getStatusConfig = (status) => STATUS[status] || STATUS.plan_to_watch;
  ...
  const onStatusChange = (animeId, val) => setWatchlistStatuses(p => ({ ...p, [animeId]: val }));
      ...
      <div className="enter-profile" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', padding: isEditing ? '34px 34px 0' : '26px 34px 0' }}>
          <div className="avatar-ring" style={{ width: 165, height: 205, border: `1.5px solid rgba(212,170,60,.36)`, position: 'relative', overflow: 'hidden', background: '#000', flexShrink: 0 }}>
            ...
            <img
              src={user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.username || 'Crew')}&background=181d2e&color=d4aa3c&size=200`}
              alt="avatar"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                ...
                <label style={{
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData(p => ({ ...p, avatarFile: e.target.files[0] || null, avatar: '' }))}
                    style={{ display: 'none' }}
                  />
                </label>
                ...
                <input name="avatar" type="text" value={formData.avatar || ''} onChange={onInput} placeholder="https://..." style={inputBase} />
              </div>
        ...
        {user.watchlist?.length > 0 ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(232px,1fr))', gap: 20 }}>
            {user.watchlist.map((anime, i) => {
              const s = getStatusConfig(watchlistStatuses[anime.animeId] || anime.status);
              return (
